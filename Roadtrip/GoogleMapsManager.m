...
- (void) getPlacesNearLatitude:(double) latitude nearLongitude:(double) longitude withRadius:(int) radius withType:(NSString *)type withCompletion: (void(^)(NSArray *placesDictionaries, NSError *error))completion {
    NSString *baseUrlString = [NSString stringWithFormat: @"https://maps.googleapis.com/maps/api/place/nearbysearch/json?radius=%d&type=%@&key=AIzaSyBNbQUYoy3xTn-270GEZKiFz9G_Q2xOOtc", radius,  type];
...
- (void) getMuseumsNearLatitude:(double) latitude nearLongitude:(double) longitude withRadius:(int) radius withCompletion: (void(^)(NSArray *placesDictionaries, NSError *error))completion {
    NSString *baseUrlString = [NSString stringWithFormat: @"https://maps.googleapis.com/maps/api/place/nearbysearch/json?radius=%d&type=museum&key=AIzaSyBNbQUYoy3xTn-270GEZKiFz9G_Q2xOOtc", radius];
    NSString *parametersString = [NSString stringWithFormat:@"&location=%f,%f", latitude, longitude];
    NSString *fullUrlString = [baseUrlString stringByAppendingString:parametersString];
    NSURL *url = [NSURL URLWithString:fullUrlString];
    NSMutableURLRequest * request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:10.0];
...
- (void) getPhotosWithReference: (NSString *) photoReference withMaxHeight: (NSString *) height withCompletion: (void(^)(NSArray *dictionaries, NSError *error)) completion {
    NSString *baseUrlString = [NSString stringWithFormat: @"https://maps.googleapis.com/maps/api/place/photo?key=AIzaSyBNbQUYoy3xTn-270GEZKiFz9G_Q2xOOtc"];
    NSString *parametersString = [NSString stringWithFormat:@"&photoreference=%@&maxheight=%@", photoReference, height];
    NSString *fullUrlString = [baseUrlString stringByAppendingString:parametersString];
    NSURL *url = [NSURL URLWithString:fullUrlString];
    NSMutableURLRequest * request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:10.0];
    NSURLSession *session = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration] delegate:nil delegateQueue:[NSOperationQueue mainQueue]];
